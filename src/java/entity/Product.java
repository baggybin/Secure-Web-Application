...
// only additional named Query get by product pattern
@Entity
@Table(name = "product")
@XmlRootElement
@NamedQueries({
    ...
    @NamedQuery(name = "Product.findByAmount", query = "SELECT p FROM Product p WHERE p.amount = :amount"),
     @NamedQuery(name="Product.getProductPattern", query="SELECT p FROM Product p WHERE p.name LIKE ?1 ORDER BY p.name ASC")
})
